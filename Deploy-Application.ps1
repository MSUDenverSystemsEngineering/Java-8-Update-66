...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$exitCode = Execute-Process -Path "jre-$appVersion-windows-i586.exe" -Parameters "/s STATIC=Disable AUTO_UPDATE=Disable WEB_JAVA=Enable WEB_ANALYTICS=Disable EULA=Disable REBOOT=Disable SPONSORS=Disable NOSTARTMENU=1" -Windows "Hidden" -WaitForMsiExec -PassThru
